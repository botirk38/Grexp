        ...
        if (currentChar == '(') {
            ...
            if (currentChar == ')') {
                ...
                if (repetitionCount > 0) {
                    std::string repetitionGroup = regex.substr(charIndexStack.top(), i - charIndexStack.top() + 1);
...
std::pair<size_t, size_t> NFA::findRepetitionCount(const std::string& regex, size_t startIndex) const {
    size_t endIndex = regex.find('}', startIndex);
    if (endIndex != std::string::npos) {
        size_t repetitionCount = std::stoul(regex.substr(startIndex + 1, endIndex - startIndex - 1));
...
std::string NFA::expandBackreferences(const std::string& regex) {
    ...
    for (unsigned int i = 0; i < regex.size(); i++) {
        ...
        if (currentChar == '(') {
            ...
            charStack.pop();
        } else if (currentChar == '\\' && (i + 1) < regex.size() && regex[i + 1] == '1') {
    ...
    for (unsigned int i = 0; i < inputLine.size(); i++) {
        ...
        for (unsigned int state : possibleStates) {
            if (state < regexLength) {
                ...
                } else if (regex[state] == '\\' && regex[state + 1] == 'd' && std::isdigit(inputLine[i])) {
